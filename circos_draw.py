...
def data_norm(chrm_name, chrm_bp_st_dic, list_of_bp_n_stats, type_of_norm):
    ...
    for i in range(length_of_list):
        if type_of_norm == "log":
            ...
            stat_val = float((math.log10(
                stat_val_list[i]) - log_min_stat_val) / (log_max_stat_val - log_min_stat_val))

        elif type_of_norm == "norm":
            stat_val = (stat_val_list[i] - min_stat_val) / \
                (max_stat_val - min_stat_val)
...
def stat_to_color(stat, type_of_norm, reverse):
    ...
    color_list = 0
    color_code = "color_list = plt.cm.%s(np.linspace(0, 1, number_of_color_breaks))" % (
        color)
    exec(color_code)
    ...
    if reverse == 'True':
        for color in reversed(color_list):
            if type_of_norm == "log":
                # normalize from 0 to 1
                normalized_val = float((math.log10(
                    stat_numer_count) - log_min_stat_val) / float(log_max_stat_val - log_min_stat_val))

            elif type_of_norm == "norm":
                normalized_val = float(
                    stat_numer_count - min_stat_val) / float(max_stat_val - min_stat_val)
...
# Draw a circle of arcs based on a list of chr which corresponed to the
# chrm size dic
...
def draw_10mb_labels(chrm_list, level):
    ...
    for chrm_name_it in chrm_list:
        ...
        five_mb_step_degree = float(
            break_size * viz_parameters['degree_per_nuc'])

        # for i number of breaks draw a line every 5mb and a
        for i in range(1, five_mb_break + 1):

            # calculate 5mb step
            working_degree = five_mb_step_degree * \
                i + viz_parameters['last_degree_end']

            global img
            # find the x and y pos of the location of the 10mb step
            sx, sy = get_x_y_coordinates(img['center_x'], img['center_y'], working_degree, viz_parameters[
                                         'rad_inner'] - viz_parameters['10mb_step_off_set'] * 1.4)

            # move to that location
            cr.move_to(sx, sy)

            # find the end of the line
            end_of_line = viz_parameters['rad_inner'] + viz_parameters[
                'ring_gap'] * level + viz_parameters['ring_width'] * (level - 1)
            sx, sy = get_x_y_coordinates(
                img['center_x'], img['center_y'], working_degree, end_of_line)
            # write line
            cr.line_to(sx, sy)
            ...
            if i % 2 == 0:
                ...
                # working_degree, viz_parameters['rad_inner'] -
                # viz_parameters['10mb_step_off_set']*1.75)
        ...
        # Update where the start of next chrm is os labeling can be indexed
        # corretly
        viz_parameters['total_degrees'] = float(
            viz_parameters['degree_per_nuc']) * float(chr_size_dic[chrm_name_it])
        viz_parameters['last_degree_end'] = float(viz_parameters['last_degree_end']) + float(
            viz_parameters['total_degrees']) + float(viz_parameters['arc_padding_in_degrees'])

# Draw chrm name labels use ither provided labels from list or generate
# new names with roman number (roman= 1)


def chrm_label(chrm_list, total_levels, roman):
    count = 0
    for chrm_name in chrm_list:
        degree_for_label = float(
            viz_parameters['degree_per_nuc']) * float(chr_size_dic[chrm_name]) / 2
        working_degree = viz_parameters['last_degree_end'] + degree_for_label
        radian_for_label = viz_parameters['rad_inner'] + viz_parameters[
            'ring_gap'] * total_levels + viz_parameters['ring_width'] * total_levels

        label_x, label_y = get_x_y_coordinates(
            img['center_x'], img['center_y'], working_degree, radian_for_label)
        # working_degree, viz_parameters['rad_inner'] -
        # viz_parameters['10mb_step_off_set']*1.75)
        label = ""
        ...
        # corretly
        viz_parameters['total_degrees'] = float(
            viz_parameters['degree_per_nuc']) * float(chr_size_dic[chrm_name])
        viz_parameters['last_degree_end'] = float(viz_parameters['last_degree_end']) + float(
            viz_parameters['total_degrees']) + float(viz_parameters['arc_padding_in_degrees'])
...
def color_key(total_levels, location, trim):  # min, max,color_start, color_end,
    ...
    global viz_parameters
    radius_key = viz_parameters['rad_inner'] + viz_parameters['ring_gap'] * total_levels + \
        viz_parameters['ring_width'] * total_levels + \
        viz_parameters['key_loc_offset']
    sx, sy = get_x_y_coordinates(img['center_x'], img[
                                 'center_y'], working_degree_key + viz_parameters['key_degree_off_set'], radius_key)
    sx_key = 0
    for i in range(total_levels):

        sx_key = sx + (viz_parameters['key_width']) * \
            i + viz_parameters['key_sep_distance'] * i
        ...
        if trim == 0:
            ...
            key = stat_list[i]
            it_y = sy
            # sort all norm values and color each line basaed on the dictionary
            # of color
            global color_stat_mapper_dic
            ...
            draw_label(stat_list[i], sx_key + viz_parameters['key_width'] / 2 + 4,
                       sy - 3, 12 * viz_parameters['key_label_font_x'], working_degree_key)

            # Draw min labels
            draw_label(str(stats_dic[stat_list[i]][0]), sx_key - 2, sy + viz_parameters[
                       'key_height'], 9 * viz_parameters['key_label_font_x'], working_degree_key)

            # Draw max label
            draw_label(str(stats_dic[stat_list[i]][
                       1]), sx_key - 2, sy, 9 * viz_parameters['key_label_font_x'], working_degree_key)
            ...
            cr.rectangle(sx_key, sy, viz_parameters[
                         'key_width'], viz_parameters['key_height'])
            cr.close_path()
            cr.set_line_width(viz_parameters['width'] - 1)
            cr.set_dash([])
            cr.set_source_rgb(0, 0, 0)
...
def chrm_arc(chrm_name, level, trim):
    # Create intial arc
    global viz_parameters
    radius = viz_parameters['rad_inner'] + viz_parameters['ring_gap'] * \
        level + viz_parameters['ring_width'] * level
    ...
    sx, sy = get_x_y_coordinates(img['center_x'], img['center_y'], viz_parameters[
                                 'last_degree_end'], radius)

    cr.move_to(sx, sy)
    start_deg = viz_parameters['last_degree_end']
    end_deg = viz_parameters['last_degree_end'] + \
        viz_parameters['total_degrees']

    # draw outer arc
    cr.new_sub_path()
    cr.arc_negative(img['center_x'], img['center_y'], radius,
                    math.radians(end_deg), math.radians(start_deg))

    # draw line to inner arc
    sx, sy = get_x_y_coordinates(img['center_x'], img['center_y'], viz_parameters[
                                 'last_degree_end'], radius - viz_parameters['ring_width'])
    cr.line_to(sx, sy)

    # draw reverse arc
    cr.arc(img['center_x'], img['center_y'], radius - viz_parameters['ring_width'],
    ...
    # Update the end of viz parameter[last_degree_end] + padding --> for next
    # arc start degree
    viz_parameters['last_degree_end'] = float(viz_parameters['last_degree_end']) + float(
        viz_parameters['total_degrees']) + float(viz_parameters['arc_padding_in_degrees'])
...
def draw_stats(chrm_list, level):
    for chrm_name in chrm_list:
        ...
        radius_stat = viz_parameters[
            'rad_inner'] + viz_parameters['ring_gap'] * level + viz_parameters['ring_width'] * level
        ...
        arc_length = float(2 * math.pi * radius_stat *
                           (viz_parameters['total_degrees'] / 360))
        ...
        for list_it in range(len(work_dic[chrm_name])):
            if level == 1:
                ...
                for smoothed_stat in smoothed_stats:
                    global img
                    sx, sy = get_x_y_coordinates(
                        img['center_x'], img['center_y'], working_degree, radius_stat)

                    cr.move_to(sx, sy)

                    line_x, line_y = get_x_y_coordinates(img['center_x'], img[
                                                         'center_y'], working_degree, radius_stat - viz_parameters['ring_width'])
                    ...
                    # following line is not my code from :
                    # http://stackoverflow.com/questions/12141150/from-list-of-integers-get-number-closest-to-a-given-value
                    closest_val = min(sorted(color_stat_mapper_dic[stat_list[level]].keys(
                    )), key=lambda x: abs(x - float(smoothed_stat)))

                    color = color_stat_mapper_dic[
                        stat_list[level]][closest_val]
                    cr.set_source_rgba(color[0], color[1], color[2], color[3])
                    cr.stroke()
            ...
            if level == 0:
                ...
                degree_for_data_pt = float(work_dic[chrm_name][list_it][
                                           0]) * viz_parameters['degree_per_nuc']
                ...
                # http://stackoverflow.com/questions/12141150/from-list-of-integers-get-number-closest-to-a-given-value
                closest_val = min(sorted(color_stat_mapper_dic[stat_list[level]].keys(
                )), key=lambda x: abs(x - float(stat_for_drawing)))
        ...
        # Update the end of viz parameter[last_degree_end] + padding --> for
        # next arc start degree

        viz_parameters['last_degree_end'] = float(viz_parameters['last_degree_end']) + float(
            viz_parameters['total_degrees']) + float(viz_parameters['arc_padding_in_degrees'])
    print "Done with level:", level
# -------- Main Drawing function -------- #
# Draw all chrm arc for a given level w (1) or wo (0) balck trim w 10mb
# labels, color
